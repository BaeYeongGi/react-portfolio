		...
		<PortfolioSlideWrap>
			<Swiper {...params}>
				<SwiperSlide>
					...
					<PortfolioTxt>
						<li>
							MouseOver → 2depth 메뉴 노출. 반응형 Navigation으로 제작되었으며 해상도에 따라 PC와 Mobile에 맞는 각각의 컴포넌트를 보여줍니다. i18next 다국어 처리 적용
							모듈을 설치하여 국문버전과 영문버전 각각의 화면을 보여줍니다.
						</li>
						<li>CSS Animation과 Transition을 이용한 동적인 효과를 구현하였습니다.</li>
						<li>
							get통신을 이용하여 Backend에서 작업한 최신뉴스 API를 불러옵니다. 기존뉴스목록을 불러오는 API에서 slice 메서드를 적용해 3개만 노출시켜 줍니다.(But 모든
							뉴스 목록을 불러오기 때문에 노출시켜야 하는 갯수의 API LIST만 별도로 가져와야 합니다.)
						</li>
				...
				<SwiperSlide>
					...
					<PortfolioTxt>
						<li>
							news 목록을 보여주는 페이지에서 뉴스를 클릭하면 기사가 존재하는 경우 해당 기사 링크로 이동하게 됩니다. 기사가 존재하지 않을경우 Backend에 입력해두었던
							기사 데이터를 불러옵니다. 제목, 내용, 날짜 등 불러오는 값이 html tag로 구성되어 있기 때문에 html-react-parser 라는 Library를 추가적으로 설치하여 페이지를
							보여줍니다.
						</li>
					</PortfolioTxt>
				</SwiperSlide>
				<SwiperSlide>
					...
					<PortfolioTxt>
						<li>
							회사의 상품을 다운로드 할 수 있는 다운로드 링크 입니다. 로그인이 되어 있는 상태일 경우에만 다운로드가 가능하게 설계되어 있습니다. 삼항연산자를 이용하여
							로그인상태와 아닌상태를 구분하여 클라이언트에게 보여줍니다. 로그인을 하지 않은 클라이언트는 로그인이 필요하다는 alert을 띄워준 후 Yes를 누를경우 Login
							페이지를 보여주고 No를 누를경우 alert이 닫힙니다.
						</li>
					</PortfolioTxt>
				</SwiperSlide>
				<SwiperSlide>
					...
					<PortfolioTxt>
						<li>
							아이디와 패스워드를 입력하여 로그인 하는 화면입니다. 입력정보를 올바르게 입력하지 않으면 로그인을 할 수 없다는 알람을 띄웁니다. 계정이 존재하지 않으면
							Sign Up을 통해 회원가입 페이지로 이동합니다.
							<br />
							로그인을 한 후 Backend로부터 accessToken과 로그인한계정의 정보를 받아 Redux에 정보를 저장합니다. accessToken과 sessionStorage를 이용하여 다른페이지로
							전환을 시도해도 회원 정보를 잃지않도록 합니다.
						</li>
					</PortfolioTxt>
				</SwiperSlide>
				<SwiperSlide>
					...
					<PortfolioTxt>
						<li>
							회원가입전 회원약관과 개인정보보호정책에 동의를 하는 화면입니다. 두 개의 checkbox를 모두 동의할 경우에만 Sign Up 버튼을 통해 다음 화면으로 넘어갑니다.
						</li>
					</PortfolioTxt>
				</SwiperSlide>
				<SwiperSlide>
					...
					<PortfolioTxt>
						<li>
							회원가입 입력 화면입니다.
							올바른 회원정보를 입력하기 위해 react-hook-form 모듈을 설치하여 조건에 맞는 validation을 추가하였습니다.
							비밀번호의 경우 정규표현식을 이용하여 적용하였습니다.
						</li>
						<li>
							Backend에서 보내준 국가코드를 출력해줍니다. get통신을 이용하였습니다.
						</li>						
					</PortfolioTxt>
				</SwiperSlide>
				<SwiperSlide>
					...
					<PortfolioTxt>
						<li>
							회원가입한 메일로 인증링크를 보내줍니다.
							메일함에서 인증 링크를 클릭시 회원가입이 완료되는 페이지를 보여줍니다. 
						</li>
					</PortfolioTxt>
				</SwiperSlide>
				<SwiperSlide>
					...
					<PortfolioTxt>
						<li>
							로그인이 된 상태라면 상단의 GNB에서 LOGIN 메뉴는 MY PAGE로 변경됩니다. 로그인 후 Redux에서 저장된 정보를 MyPage 컴포넌트에 출력해줍니다. 
						</li>
						<li>
				...
				<SwiperSlide>
					...
					<PortfolioTxt>
						<li>
							로그인한 계정의 정보를 수정하여 적용하는 API를 사용합니다. 
						</li>
					</PortfolioTxt>
				</SwiperSlide>
				<SwiperSlide>
				...
				<SwiperSlide>
					...
					<PortfolioTxt>
						...
						<li>현재페이지의 Index 값을 이용하여 Pagination을 구현합니다. 페이지가 첫번째 혹은 마지막번째 페이지일 경우 필요없는 Controller를 지우는 코드를 함께 작성해줍니다.</li>
